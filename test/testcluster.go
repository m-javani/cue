...
func NewTestCluster(ctx context.Context, opts ...Option) (*Cluster, error) {
	...
	caInfo, err := testutils.LoadCA(o.certsDir, "ca")
	if err != nil {
	...
	for _, name := range nodeNames {
		_, _, err = testutils.CreateNodeCert(tmpDir, caInfo, testutils.NodeCert{
			NodeIdentity: name,
...
func startNode(
	...
	authPath := "/opt/cue/configs/auth.yml"
	...
	for _, f := range certEntries {
		...
		files = append(files, testcontainers.ContainerFile{
			...
			ContainerFilePath: "/opt/cue/certs/" + f.Name(),
			FileMode:          0644,
		})
	}
	...
	req := testcontainers.ContainerRequest{
		...
		Cmd: []string{
			"serve",
			"--config", "/opt/cue/configs/config.yml",
			"--node-id", name,
			"--data-dir", "/opt/cue/data",
			"--cluster-cert", fmt.Sprintf("/opt/cue/certs/%s.pem", name),
			"--cluster-key", fmt.Sprintf("/opt/cue/certs/%s_key.pem", name),
			"--cluster-ca", "/opt/cue/certs/ca_cert.pem",
			"--proxy-cert", fmt.Sprintf("/opt/cue/certs/%s.pem", name),
...
func ProbeNode(ctx context.Context, addr string) (*NodeProbeResult, error) {
	const (
		maxRetries = 10 // More retries for leader election
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// Log attempt for debugging (optional)
		// log.Printf("Probe attempt %d/%d for %s", attempt+1, maxRetries+1, addr)

		result, err := probeOnce(ctx, client, addr)
		if err != nil {
			lastErr = err
			...
			// Check if LeaderID is set
			if result.Cluster.LeaderID != "" {
				return result, nil
			}

			// LeaderID is empty, we need to retry
			lastErr = fmt.Errorf("leader not elected yet (attempt %d)", attempt+1)
	...
	return nil, fmt.Errorf("probe failed after %d attempts: %w", maxRetries+1, lastErr)
}

func probeOnce(ctx context.Context, client *http.Client, addr string) (*NodeProbeResult, error) {
...
func isRetryableError(err error) bool {
	...
	// Check for context deadline exceeded (but not cancellation)
	if errors.Is(err, context.DeadlineExceeded) {
		return true
	}

	// Check for HTTP errors that might be temporary
