...
type NodeCert struct {
	...
	ServerNames  []string // SAN DNS names, e.g. "node1.localhost", "localhost"
}
...
// The certificate includes SAN entries (node.localhost, localhost, and 127.0.0.1) by default.
...
func CreateNodeCert(dir string, ca *CAInfo, node NodeCert, yearsValid int) (certPath, keyPath string, err error) {
	if ca == nil {
		return "", "", fmt.Errorf("CA info is nil")
	}
	if ca.Certificate == nil {
		return "", "", fmt.Errorf("CA certificate is nil")
	}
	if ca.PrivateKey == nil {
		return "", "", fmt.Errorf("CA private key is nil")
	}
	if ca.CertPath == "" {
